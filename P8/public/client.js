...
        const response = await fetch(`/api/lobby/join`, { method: 'POST' });
...
        }
    } catch (error) {
        console.error("Fallo al hacer polling al Matchmaker", error);
    }
}
...
        await fetch(`/api/game/${myRoomId}/setup`, {
            method: 'POST',
...
    try {
        const response = await fetch(`/api/game/${myRoomId}/state/${myPlayerNumber}`);
...
    try {
        const response = await fetch(`/api/game/${myRoomId}/shoot`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
